...
def build_mzqc(run_data: List[Dict[str, Any]]) -> str:
    ...
    for idx, run_info in enumerate(run_data, 1):
        ...
        for k, v in metrics_dict.items():
            # ensure JSON-serialisable value; n-tuples (lists/tuples) and tables
            # (dicts of columns) are kept as JSON arrays/objects with non-finite
            # entries mapped to null, not stringified
            def _clean(x):
            ...
            if v is None or (isinstance(v, float) and not np.isfinite(v)):
                ...
                val = [_clean(x) for x in v]
            elif isinstance(v, dict):
                val = {k2: ([_clean(x) for x in col] if isinstance(col, (list, tuple)) else _clean(col))
                       for k2, col in v.items()}
            else:
                val = str(v)
